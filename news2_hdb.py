...
def resetear():
    # Poner de nuevo los valores por defecto de cada widget
    st.session_state["a"] = None      # soporte O2
    st.session_state["b"] = 15        # FR
    st.session_state["c"] = None      # EPOC
    st.session_state["d"] = 95        # SpO2
    st.session_state["e"] = 120       # PAs
    st.session_state["f"] = 80        # FC
...
if st.session_state.get("score") is not None:
    st.markdown(
        f'<div class="main-result">{st.session_state["score"]}</div>',
        unsafe_allow_html=True
    )
